...
public class StringEx {
	...
	private static void stringMethods() {
		...
		//검색 관련 : 문자, 문자열의 위치 찾을떄
		System.out.println("5번 인덱스의 문자 : " + s.charAt(5));
		//중요! indexOf
		System.out.println("Fun 문자열의 위치 인덱스 : " + s.indexOf("Fun")); //검색어 대소문자를 구분
		System.out.println("fun 문자열의 위치 인덱스 : " + s.indexOf("fun")); // -1 : 찾을 수 없음
		System.out.println("m의 위치 인덱스 : " + s.indexOf("m")); // 11
		System.out.println("m의 역방향 위치 인덱스 : " + s.lastIndexOf("m"));
		System.out.println("m의 위치 인덱스 2nd : " + s.indexOf("m", 12));// 검색시작 위치를 12번 인덱스 부터
		...
		//문자열의 대소비교 : 유니코드 값의 대소비교
		//ABC < ABD
		//같으면 0, 비교값이 작으면 음수, 크면 양수
	...
	private static void stringBasic() {
		...
		//기본 타입을 문자열 개체로 변환 : valueof
		String s5 = String.valueOf(Math.PI);
		System.out.println("s5 = " + s5);
	}
	
	private static void stringBufferEx() {
		//복잡한 문자열을 가공할 때 매번 새로운 문자열 객체 생성 -> 내부에 버퍼(임시저장공간)를 가진 StringBuffer 활용, 메모리 관리에 용이
