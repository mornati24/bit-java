...
public class ArraysEx {
	...
	private static void customSearch() {
		Member[] members = {
				new Member("홍길동"), new Member("고길동"), new Member("임꺽정"), new Member("장길산")
		};
		System.out.println("원본배열 : " + Arrays.toString(members));
//		Arrays.sort(members);
		System.out.println("정렬된 배열 : " + Arrays.toString(members));
		int index = Arrays.binarySearch(members, new Member("장길산")); //사용자 정의 객체의 검색시에는 객체를 넘겨주어 해당 객체와 같은 지를 비교해주어야 한다
		System.out.println("검색된 인덱스 : " + index);
		
	}
	//검색 테스트
	private static void basicSearch() {
		// java는 기본적으로 binary search방식을 채택함
		// 미리 데이터가 정렬되어 있어야 한다
		// 기본 자료형 검색
		...
		System.out.println("원본배열 : " + Arrays.toString(items));
		//미리 데이터가 정렬되어 있어야 함(중요!)
		Arrays.sort(items);
	...
	//역순 정렬 - 내림차순(Descending : 큰값 -> 작은 값 순)
